...
from geometry_msgs.msg import TransformStamped, Point, Pose, Twist, Quaternion, Vector3
...
class RobotinoOdometryMonitor(Node):
    def __init__(self):
        ...
        # Publishers
        self.odom_publisher = self.create_publisher(Odometry, 'robotino/odom', 10)
        
        # TF Broadcaster
        self.tf_broadcaster = TransformBroadcaster(self)
        
        # Timer
        self.timer = self.create_timer(1.0 / self.update_rate, self.publish_odometry)
    ...
    def get_odometry_data(self) -> Optional[List[float]]:
        """Get odometry data from Robotino API"""
        try:
            ...
            if len(data) != 7:  # [x, y, phi, vx, vy, omega, seq]
                raise ValueError(f"Invalid odometry data format: {data}")
    ...
    def create_odometry_message(self, data: List[float]) -> Tuple[Odometry, TransformStamped]:
        ...
        # Set velocities
        odom.twist.twist.linear = Vector3(x=float(data[3]), y=float(data[4]), z=0.0)
